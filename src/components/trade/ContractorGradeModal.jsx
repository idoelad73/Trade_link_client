...
import { uploadContractorGradePhoto } from '../../api/trade.js';
...
export default function ContractorGradeModal({ contractor, onSubmit, onClose }) {
  ...
  async function handlePhotoAdd(e) {
    ...
    try {
      const url = await uploadContractorGradePhoto(file);
      ...
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="px-7 pt-6 pb-7 overflow-y-auto">
          ...
          <div className="flex items-center gap-4 mb-5">
            <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-sky-100 to-emerald-100 flex items-center justify-center text-2xl flex-shrink-0">🏢</div>
            <div className="min-w-0">
              <p className="font-extrabold text-slate-800 text-base truncate">{contractor.contractor_name}</p>
              <p className="text-xs font-semibold text-emerald-600 mt-0.5">Contractor</p>
              {contractor.site_name && <p className="text-xs text-slate-400 truncate mt-0.5">📍 {contractor.site_name}</p>}
            </div>
          </div>

          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4 text-center">Rate this contractor</p>
          ...
          <div className="text-center h-8 mb-5">
            {info
              ? <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-sm font-bold" style={{ background: info.color + '18', color: info.color }}>{info.emoji} {info.label}</span>
          ...
          <div className="mb-4">
            <textarea
              ...
              placeholder="Share your experience with this contractor (optional)…"
          ...
          <div className="mb-5">
            ...
            <div className="flex flex-wrap gap-2">
              {photos.map(p => (
                <div key={p.tmpId} className="relative w-16 h-16 rounded-xl overflow-hidden border border-slate-200 bg-slate-100 flex-shrink-0">
                  {p.uploading
                    ? <div className="w-full h-full flex items-center justify-center"><div className="w-5 h-5 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" /></div>
                    : <>
                        <img src={p.url} alt="" className="w-full h-full object-cover" />
                        <button type="button" onClick={() => handlePhotoRemove(p.tmpId)}
                          className="absolute top-0.5 right-0.5 w-4 h-4 rounded-full bg-red-500 text-white flex items-center justify-center text-[9px] font-bold hover:bg-red-600">✕</button>
                      </>
                  }
                </div>
              ))}
            ...
            <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={handlePhotoAdd} />
          </div>
