...
export default function TradeReviewsPage() {
  ...
  if (error || !data) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-gradient-to-br from-slate-50 to-indigo-50">
  ...
  const gradeInfo = pro.avgGrade ? GRADE_LABELS[Math.round(pro.avgGrade)] : null;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 pb-16">
      ...
      <div className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-slate-100 px-4 py-3 flex items-center gap-3 shadow-sm">
        ...
        <span className="font-bold text-slate-700 text-sm truncate">Reviews — {pro.fullName}</span>
      </div>
      ...
      <div className="max-w-xl mx-auto px-4 pt-6">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 mb-6 flex items-center gap-4">
          {pro.photo
            ? <img src={pro.photo} alt={pro.fullName} className="w-16 h-16 rounded-2xl object-cover border border-slate-100 flex-shrink-0" />
            : <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-sky-100 to-amber-100 flex items-center justify-center text-3xl flex-shrink-0">👤</div>
          }
          <div className="flex-1 min-w-0">
            <p className="font-extrabold text-slate-800 text-lg truncate">{pro.fullName}</p>
            <p className="text-sm font-semibold text-sky-600">{pro.professionality}</p>
            {pro.avgGrade ? (
              <div className="flex items-center gap-2 mt-1">
                <StarRow avg={pro.avgGrade} size={16} />
                <span className="text-sm font-extrabold text-slate-700">{pro.avgGrade.toFixed(1)}</span>
                <span className="text-xs text-slate-400">({pro.gradeCount} review{pro.gradeCount !== 1 ? 's' : ''})</span>
              </div>
            ) : (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {r.photos.map((url, idx) => (
                        <button
                          ...
                          className="w-16 h-16 rounded-xl overflow-hidden border border-slate-200 flex-shrink-0 hover:opacity-90 transition focus:outline-none focus:ring-2 focus:ring-indigo-300"
                        >
        ...
        >
          ...
          {lightbox.photos.length > 1 && (
            <>
              ...
              >‹</button>
              <button
                className="absolute right-4 text-white text-3xl font-bold hover:text-slateite-300 select-none"
